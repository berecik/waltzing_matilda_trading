...
class TestAsyncViews(TestCase):
    ...
    async def test_place_order(self):
        payload = {"stock_id": self.stock.id, "quantity": 10, "order_type": "buy"}
    ...
    async def test_total_investment(self):
        ...
        await sync_to_async(Order.objects.create)(
            user=self.user, stock=self.stock, quantity=10, order_type="buy"
        )
        await sync_to_async(Order.objects.create)(
            user=self.user, stock=self.stock, quantity=5, order_type="sell"
        )
    ...
    async def test_invalid_token(self):
        ...
        response = await self.client.post(
            "/orders/order", json=payload, headers=invalid_auth_header
        )
