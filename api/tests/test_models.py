...
def test_stock_sum():
    ...
    order2 = Order.objects.create(
        user=user, stock=stock, quantity=10, order_type="sell"
    )
    order3 = Order.objects.create(
        user=user, stock=stock, quantity=4, order_type="buy"
    )
    order4 = Order.objects.create(
        user=user, stock=stock, quantity=6, order_type="buy"
    )
...
def test_order_quantity_validation():
    ...
    with pytest.raises(IntegrityError):
        Order.objects.create(user=user, stock=stock, quantity=-5, order_type="buy")
...
def test_order_type_validation():
    ...
    with pytest.raises(ValidationError):
        order = Order(user=user, stock=stock, quantity=10, order_type="invalid_type")
        order.full_clean()
